...
parser.add_argument('--epochs', type=int, default=17, help='number of epochs to train')
...
def test(model, model_loss, TestImgLoader, args):
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
        start_time = time.time()
        loss, scalar_outputs, image_outputs = test_sample_depth(model, model_loss, sample, args)
        depth_loss=scalar_outputs['depth_loss']
        #save_scalars(logger, 'test', scalar_outputs, batch_idx)
        #save_images(logger, 'test', image_outputs, batch_idx)
        avg_test_scalars.update(scalar_outputs)
        del scalar_outputs, image_outputs
        if (not is_distributed) or (dist.get_rank() == 0):
            print('Iter {}/{}, test loss = {:.3f}, depth_loss = {:.3f},time = {:3f}'.format(batch_idx, len(TestImgLoader), loss,depth_loss,
                                                                        time.time() - start_time))
            if batch_idx % 100 == 0:
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    #print(sample_cuda["filename"][0].format('img', 'abserror:'),"{}".format(AbsDepthError_metrics(depth_est, depth_gt, mask > 0.5)))
